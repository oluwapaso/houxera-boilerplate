...
const OurServicesPageVar1 = ({ is_theme = false, size = 20, raw_data = {} }: { is_theme?: boolean, size?: number, raw_data?: any }) => {
                        ...
                        {(!loading && serviceError == "" && Array.isArray(services)) &&
                            <div className="grid grid-cols-1 sm:grid-cols-1 lg:grid-cols-3 gap-6 sm:gap-8 mb-8 sm:mb-12">
                                {services.map((srvc, index) => (
                                    <OurServicesCardVar1 key={srvc.service_uid} is_theme={is_theme} service_info={srvc} />
                                ))}
                            </div>
                        }
...
export default OurServicesPageVar1